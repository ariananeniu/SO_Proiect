...
int main(int argc, char *argv[]) {
    ...
    // Gestionarea Link-ului Simbolic la fiecare rulare
    ...
    if (strcmp(command, "add") == 0) {
        ...
        if (arg_idx + 2 < argc) remove_report(district, atoi(argv[arg_idx + 2]));
        else printf("Argument lipsa: ID raport.\n");
    }
    else if (strcmp(command, "update_threshold") == 0) {
        if (strcmp(role, "manager") != 0) {
            printf("Doar managerul poate actualiza pragul.\n");
            return 1;
        }
        if (arg_idx + 2 < argc) update_threshold(district, argv[arg_idx + 2]);
        else printf("Argument lipsa: valoare prag.\n");
    }
